...
function antiTamperGuard(bundlePath) {
    ...
    // In bridge mode the bundle is written to a temp location by the Zig process,
    // so skip the path locality check — the path is trusted via env var.
    if (process.env.CHRONOS_BRIDGE_MODE !== '1') {
...
function createInternalLoader(manifest) {
    ...
    function resolveInternal(fromId, request) {
        ...
        for (const candidate of candidates) {
            if (Object.prototype.hasOwnProperty.call(modules, candidate) || cache.has(candidate)) {
                return candidate;
            }
        }
